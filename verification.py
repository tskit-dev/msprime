...
class SimulationVerifier:
    ...
    def _run_seq_gen(self, tree, args):
        ts = tree.tree_sequence
        labels = {u: str(u) for u in ts.samples()}
        newick = tree.newick(node_labels=labels)
        ...
        with tempfile.TemporaryFile("w+") as in_file, tempfile.TemporaryFile(
            ...
            for line in out_file:
                ...
                sequences[int(sample_id)] = sequence
                assert len(sequence) == ts.sequence_length
        assert len(sequences) == ts.num_samples

        # Now we've got the sequences we can make a matrix of them and pass them to
        # scikit-allel somehow. Then we compute some stats, and pass these back?
        # Or is there some other way to compute stats?

        # genotype_matrix = np.array((ts.sequence_length, ts.num_samples), dtype=int)
        # for j, u in enumerate(ts.samples()):
        #     print(j, u, sequences[u], s)

    def _run_seq_gen_msprime_comparison(self, model, length=100):
        """
        Runs a comparison between mutations generated by msprime and seq_gen
        for the specified model.
        """
        # NOTE: this is under development and will hopefully be filled out
        # soon with lots of tests that check the mutation models in msprime.
        # We're ignoring the model for now, but we'll need some mapping
        # between the parameters for msprime and Seq-Gen.
        ts = msprime.simulate(10, Ne=10 ** 4, length=length)
        args = ["-q", "-m", "HKY", "-l", str(length)]
        df_sg = self._run_seq_gen(ts.first(), args)
        print(df_sg)

        # TODO how do we make the rates in msprime and SeqGen line up?
        ts_mutated = msprime.mutate(ts, rate=1e-8, model=msprime.JukesCantor())
        print(ts_mutated)
    ...
    def run_pi_analytical_check(self):
        ...
        for k, n in enumerate(sample_size):
            ...
            mean[k] = np.mean(pi)
            var[k] = np.var(pi)
            print(n, theta, np.mean(pi), predicted_var[k], np.var(pi), sep="\t")
...
def run_tests(args):
    ...
    verifier.add_ms_instance("size-change1", "10 10000 -t 2.0 -eN 0.1 2.0")
    verifier.add_ms_instance("growth-rate-change1", "10 10000 -t 2.0 -eG 0.1 5.0")
    ...
    verifier.add_ms_instance("gene-conversion-2", "100 10000 -t 5.0 -r 10 2501 -c 2 1")
    verifier.add_ms_instance(
    ...
    verifier.add_ms_instance("msdoc-simple-ex", "4 20000 -t 5.0")
    verifier.add_ms_instance("msdoc-recomb-ex", "15 1000 -t 10.04 -r 100.0 2501")
    verifier.add_ms_instance("msdoc-structure-ex1", "15 1000 -t 2.0 -I 3 10 4 1 5.0")
    verifier.add_ms_instance(
    ...
    verifier.add_discoal_instance("discoal-simple-ex", "15 1000 100 -t 5.0")
    ...
    # Seq-gen vs msprime tests.
    verifier.add_seq_gen("JC69")
