...
def mutate(tree_sequence, rate=None, random_seed=None, model=None, keep=False):
    ...
    By default, sites and mutations in the parameter tree sequence are discarded.
    If the ``keep`` parameter is true, however, *additional* mutations are
    simulated. To ensure that the returned tree sequence is valid, and
    mutations generated by this function that have precisely the same position
    are discarded.
