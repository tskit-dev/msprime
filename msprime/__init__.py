...
def simulate_trees(sample_size, num_loci, recombination_rate,
    ...
    try:
        ...
        del sim
        tree_reader = TreeReader(tf)
        for l, pi, tau in tree_reader:
            yield l, pi, tau
    finally:
        os.unlink(tf)
...
class TreeReader(object):
    ...
    def __init__(self, tree_file_name):
        self._ll_tree_reader = _msprime.TreeReader(tree_file_name)

    def get_sample_size(self):
        return self._ll_tree_reader.get_sample_size()

    def get_num_loci(self):
        return self._ll_tree_reader.get_num_loci()

    def get_num_trees(self):
        return self._ll_tree_reader.get_num_trees()

    def get_metadata(self):
        return self._ll_tree_reader.get_metadata()

    def __iter__(self):
        for j in range(self.get_num_trees()):
            yield self._ll_tree_reader.get_tree(j)
