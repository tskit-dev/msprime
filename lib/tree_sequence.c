...
int WARN_UNUSED
tree_sequence_compute_mutation_parents(tree_sequence_t *self, mutation_id_t *parent,
        int flags)
{
    int ret = 0;
    int f_ret;
    ...
    memset(&tree, 0, sizeof(sparse_tree_t));
    bottom_mutation = malloc(self->nodes.num_records * sizeof(*bottom_mutation));
    if (bottom_mutation == NULL) {
        ret = MSP_ERR_NO_MEMORY;
        goto out;
    }
    ...
    /* Everything is set to NULL_MUTATION initially */
    memset(parent, 0xff, num_mutations * sizeof(*parent));
    memset(bottom_mutation, 0xff, self->nodes.num_records * sizeof(*bottom_mutation));
    for (f_ret = sparse_tree_first(&tree); f_ret == 1; f_ret = sparse_tree_next(&tree)) {
        ret = sparse_tree_get_sites(&tree, &sites, &num_sites);
        if (ret != 0) {
            goto out;
        }
        for (j = 0; j < num_sites; j++) {
            ...
            for (k = 0; k < sites[j].mutations_length; k++) {
                mutation = sites[j].mutations[k];
                if (bottom_mutation[mutation.node] != MSP_NULL_MUTATION) {
                    parent[mutation.id] = bottom_mutation[mutation.node];
                }
            ...
            for (k = 1; k < sites[j].mutations_length; k++) {
                mutation = sites[j].mutations[k];
                if (parent[mutation.id] == MSP_NULL_MUTATION) {
                    u = tree.parent[mutation.node];
                    while (u != MSP_NULL_NODE && bottom_mutation[u] == MSP_NULL_MUTATION) {
                        u = tree.parent[u];
                    }
                    if (u != MSP_NULL_NODE) {
                        parent[mutation.id] = bottom_mutation[u];
                    }
                }
            }
            ...
            for (k = 0; k < sites[j].mutations_length; k++) {
                mutation = sites[j].mutations[k];
                bottom_mutation[mutation.node] = MSP_NULL_MUTATION;
            }
        }
    }
    if (f_ret != 0) {
