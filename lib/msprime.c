...
msp_alloc_segment(msp_t *self, uint32_t left, uint32_t right, node_id_t value,
        population_id_t population_id, label_id_t label, segment_t *prev, segment_t *next)
{
    segment_t *seg = NULL;
...
msp_alloc(msp_t *self,
        ...
        recomb_map_t *recomb_map, tree_sequence_t *from_ts, size_t num_labels, gsl_rng *rng) {
    int ret = -1;
    size_t j;
...
msp_insert_individual(msp_t *self, segment_t *u)
{
    ...
    node = avl_insert_node(
        &self->populations[u->population].ancestors[u->label], node);
    assert(node != NULL);
out:
    return ret;
}
...
msp_print_state(msp_t *self, FILE *out)
{
    ...
    fprintf(out, "Population configurations\n");
    for (j = 0; j < self->num_populations; j++) {
        fprintf(out, "pop[%d]:\n", (int) j);
    ...
    while (x != NULL) {
        ...
        if (x->right > k) {
            ...
            z = msp_alloc_segment(self, (uint32_t) k, x->right, x->value,
                    x->population, x->label, seg_tails[ix], x->next);
    ...
    while (avl_count(Q) > 0) {
        ...
        if (h == 1) {
            ...
            if (nm->value == h) {
                ...
                alpha = msp_alloc_segment(self, l, r, v, population_id, 0,
                        NULL, NULL);
                if (alpha == NULL) {
                    ret = MSP_ERR_NO_MEMORY;
                    goto out;
    ...
    for (root = 0; root < (node_id_t) num_nodes; root++) {
        ...
        if (seg != NULL) {
            ...
            for (seg = seg->next; seg != NULL; seg = seg->next) {
                fenwick_set_value(&self->links[label], seg->id, seg->right - seg->prev->right);
            }
        }
    }
...
msp_initialise(msp_t *self)
{
    ...
    /* single_sweep model will need to be treated differently here
     * as we're allocing space to store the trajectory?? */
...
msp_store_simultaneous_migration_events(msp_t *self, avl_tree_t *nodes,
    ...
    avl_node_t *node;
    avl_tree_t *source;
    label_id_t label = 0; //currently only working for label 0
...
msp_change_label(msp_t *self, segment_t *ind, label_id_t label)
{
    ...
    assert(node != NULL);
    ret = msp_move_individual(self, node, pop, ind->population, 1);
    return ret;
}
...
static double
msp_dirac_get_common_ancestor_waiting_time(msp_t *self, population_id_t pop_id, label_id_t label)
{
    population_t *pop = &self->populations[pop_id];
        ...
        label_id_t label)
{
    ...
    if (gsl_rng_uniform(self->rng) < (1 / (1.0 + self->model.params.dirac_coalescent.c))) {
        ...
        msp_free_avl_node(self, x_node);
        msp_free_avl_node(self, y_node);
        ret = msp_merge_two_ancestors(self, 0, label, x, y);
    } else {
        for (j = 0; j < 4; j++){
        ...
        for (j = 0; j < 4; j++){
            ...
            ret = msp_merge_ancestors(self, &Q[j], 0, label);
            if (ret < 0) {
                goto out;
            }
        }
...
static double
msp_beta_get_common_ancestor_waiting_time(msp_t *self, population_id_t pop_id, label_id_t label)
{
    int ret = 0;
