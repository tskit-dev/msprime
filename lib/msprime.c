...
segment_get_hull(segment_t *seg)
{
    ...
    hull = seg->hull;
    tsk_bug_assert(hull->lineage == seg);
...
static int
msp_alloc_memory_blocks_hulls(msp_t *self)
{
    int ret = 0;
    uint32_t j;

    /* allocate the hulls */
    /* Need to check here whether an object_heap
...
int
msp_set_hull_block_size(msp_t *self, size_t block_size)
{
    int ret = 0;
    ...
    self->hull_block_size = block_size;
out:
    return ret;
}
...
msp_alloc_hull(msp_t *self, double left, double right, segment_t *lineage)
{
    hull_t *hull = NULL;
    label_id_t label;
    uint32_t j;
    ...
    if (object_heap_empty(&self->hull_heap[label])) {
        ...
        /* check pointer logic here */
        for (j = 0; j < self->num_populations; j++) {
            if (self->populations[j].coal_mass_index != NULL) {
                if (fenwick_expand(&self->populations[j].coal_mass_index[label],
                        self->hull_block_size)
                    != 0) {
                    goto out;
                }
            }
    ...
    hull = (hull_t *) object_heap_alloc_object(&self->hull_heap[label]);
    if (hull == NULL) {
        goto out;
    }
    tsk_bug_assert(left < right);
    /* this is the limit of the number of hulls in the avl_tree */
    ...
    tsk_bug_assert(lineage->prev == NULL);
    lineage->hull = hull;
...
msp_alloc_memory_blocks(msp_t *self)
{
    ...
    /* Allocate the hull heaps */
    ret = msp_alloc_memory_blocks_hulls(self);
    if (ret != 0) {
        goto out;
    }
...
msp_free(msp_t *self)
{
    ...
    for (j = 0; j < self->num_populations; j++) {
        ...
        for (k = 0; k < self->num_labels; k++) {
            if (self->populations[j].coal_mass_index != NULL) {
                fenwick_free(&self->populations[j].coal_mass_index[k]);
            }
        }
        msp_safe_free(self->populations[j].coal_mass_index);
    }
...
/*
 * Returns the hull with the specified id.
 */
static hull_t *
msp_get_hull(msp_t *self, size_t id, label_id_t label)
{
    hull_t *u = object_heap_get_object(&self->hull_heap[label], id - 1);
...
static void
msp_free_hull(msp_t *self, hull_t *hull, population_id_t pop, label_id_t label)
{
    object_heap_free_object(&self->hull_heap[label], hull);
    tsk_bug_assert(self->populations[pop].coal_mass_index != NULL);
    fenwick_set_value(&self->populations[pop].coal_mass_index[label], hull->id, 0);
}
...
static inline void
hull_adjust_insertion_order(hull_t *h, avl_node_t *node)
{
    hull_t *prev_hull;
...
msp_insert_hull(msp_t *self, hull_t *hull)
{
    ...
    hull_adjust_insertion_order(hull, node);
    num_starting_before_left = (uint64_t) avl_index(node);

    /* adjust num_coalescing_pairs for all other lineages in avl */
    node = node->next;
    while (node != NULL) {
...
msp_remove_hull(msp_t *self, hull_t *hull)
{
    ...
    /* remove node from hulls_left */
    fenwick_set_value(coal_mass_index, hull->id, 0);
...
msp_verify_segments(msp_t *self, bool verify_breakpoints)
{
    ...
    if (self->model.type == MSP_MODEL_SMC_K) {
        for (j = 0; j < self->num_populations; j++) {
            for (k = 0; k < self->num_labels; k++) {
                hull_avl_nodes += avl_count(&self->populations[j].hulls_left[k]);
                hull_avl_nodes += avl_count(&self->populations[j].hulls_right[k]);
            }
        }
    }
        ...
        for (label_id = 0; label_id < (label_id_t) self->num_labels; label_id++) {
            ...
            for (a = avl->head; a->next != NULL; a = a->next) {
                ...
                for (b = a->next; b != NULL; b = b->next) {
                    ...
                    hull_b.right = GSL_MIN(
                        y->right + self->model.params.smc_k_coalescent.hull_offset,
                        self->sequence_length);
                    if (hull_a.left < hull_b.right && hull_b.left < hull_a.right) {
                        count++;
                    }
                }
            }
            /* sum all counts of hulls_left */
            avl = &self->populations[population_id].hulls_left[label_id];
            pos = -1;
            io = 0;
            for (a = avl->head; a != NULL; a = a->next) {
                hull = (hull_t *) a->item;
                num_coalescing_pairs += (int) hull->count;
            ...
            coal_mass_index
                = &self->populations[population_id].coal_mass_index[label_id];
            num_coalescing_pairs = (int) fenwick_get_total(coal_mass_index);
            tsk_bug_assert(count == num_coalescing_pairs);

            /* checks for hulls_right */
            avl = &self->populations[population_id].hulls_right[label_id];
            pos = -1;
            io = 0;
            for (a = avl->head; a != NULL; a = a->next) {
    ...
    population_id_t dest_pop, label_id_t dest_label)
{
    ...
    hull = NULL;
    if (self->model.type == MSP_MODEL_SMC_K) {
        hull = segment_get_hull(ind);
        tsk_bug_assert(hull != NULL);
        msp_remove_hull(self, hull);
    }
    ...
    if (ind->label == dest_label) {
        ...
        new_hull = NULL;
        tsk_bug_assert(hull == NULL);
        /* general multi-label case for smc_k */
        // if (hull != NULL) {
        ...
        for (label = 0; label < (label_id_t) self->num_labels; label++) {
            ...
            if (pop->hulls_left != NULL) {
                for (node = pop->hulls_left[label].head; node != NULL;
                     node = node->next) {
                    x = (hull_t *) node->item;
                    msp_free_hull(self, x, (population_id_t) j, label);
                    avl_unlink_node(&pop->hulls_left[label], node);
                    msp_free_avl_node(self, node);
                }
            }
            if (pop->hulls_right != NULL) {
                for (node = pop->hulls_right[label].head; node != NULL;
                     node = node->next) {
                    y = (hullend_t *) node->item;
                    msp_free_hullend(self, y, label);
                    avl_unlink_node(&pop->hulls_right[label], node);
                    msp_free_avl_node(self, node);
                }
            }
        }
...
msp_hulls_init_counts(msp_t *self, population_id_t pop, label_id_t label)
{
    int ret = 0;
    avl_tree_t *hulls_left, *hulls_right;
    avl_node_t *node_left, *node_right, *search;
    hullend_t hullend_target;
    hullend_t *hullend, *floor_ptr;
    hull_t *hull;
    fenwick_t *coal_mass;
    uint64_t visited, num_ending_before_hull, value;
    double left;

    visited = 0;
    hulls_left = &self->populations[pop].hulls_left[label];
    hulls_right = &self->populations[pop].hulls_right[label];
    coal_mass = &self->populations[pop].coal_mass_index[label];

    /* iterate over avl nodes */
    for (node_left = hulls_left->head; node_left != NULL; node_left = node_left->next) {
        tsk_bug_assert(avl_count(hulls_left) > 0);
        num_ending_before_hull = 0;
        hull = (hull_t *) node_left->item;
        left = hull->left;
        hullend_target.position = left;
        hullend_target.insertion_order = UINT64_MAX;
        avl_search_closest(hulls_right, &hullend_target, &search);
        if (search != NULL) {
            floor_ptr = (hullend_t *) search->item;
            if (floor_ptr->position > left) {
                search = search->prev;
            }
            if (search != NULL) {
                num_ending_before_hull = (uint64_t) avl_index(search) + 1;
            }
        }
        ...
        avl_init_node(node_right, hullend);
        node_right = avl_insert_node(hulls_right, node_right);
        tsk_bug_assert(node_right != NULL);
        hullend_adjust_insertion_order(hullend, node_right);
    }
out:
    return ret;
}
...
msp_initialise_smc_k(msp_t *self)
{
    ...
    population_id_t population_id;
    label_id_t label_id;
    avl_tree_t *population_ancestors, *hulls_left;
    avl_node_t *h_node, *a_node;
    hull_t *hull;
    double left, right;
    segment_t *seg, *head;

    for (population_id = 0; population_id < (population_id_t) self->num_populations;
         population_id++) {
        for (label_id = 0; label_id < (label_id_t) self->num_labels; label_id++) {
            population_ancestors = &self->populations[population_id].ancestors[label_id];
            hulls_left = &self->populations[population_id].hulls_left[label_id];
            for (a_node = population_ancestors->head; a_node != NULL;
                 a_node = a_node->next) {
                seg = (segment_t *) a_node->item;
                ...
                avl_init_node(h_node, hull);
                h_node = avl_insert_node(hulls_left, h_node);
                tsk_bug_assert(h_node != NULL);
                hull_adjust_insertion_order(hull, h_node);
            }
            ret = msp_hulls_init_counts(self, population_id, label_id);
            if (ret != 0) {
                goto out;
            }
        }
...
/* Recombination event during a sweep within a specified background
 */
static int
msp_sweep_recombination_event(msp_t *self, label_id_t label, double sweep_locus,
    double *mutant_population_frequency)
{
    ...
    if (label == 0) {
        if (sweep_locus < rhs->left) {
            if (r < mutant_population_frequency[rhs->population]) {
                ret = msp_change_label(self, rhs, 1);
                if (ret != 0) {
                    goto out;
                }
            }
        } else {
            if (r < mutant_population_frequency[lhs->population]) {
                ret = msp_change_label(self, lhs, 1);
                if (ret != 0) {
                    goto out;
                }
            }
    ...
    if (label == 1) {
        if (sweep_locus < rhs->left) {
            if (r < 1.0 - mutant_population_frequency[rhs->population]) {
                ret = msp_change_label(self, rhs, 0);
                if (ret != 0) {
                    goto out;
                }
            }
        } else {
            if (r < 1.0 - mutant_population_frequency[lhs->population]) {
                ret = msp_change_label(self, lhs, 0);
                if (ret != 0) {
                    goto out;
                }
            }
...
static int
genic_selection_read_trajectory(FILE *file, size_t *num_steps_ret, int *num_demes_ret, int *tot_pop_ret, double *migration_rate_ret, 
    int **final_mut_pop_ret, int **mut_pop_ret, double **allele_frequency_mut_ret, double **t_of_forward_ev_ret, int **ev_type_ret, int **start_deme_ret,
        int **end_deme_ret)
{
    int ret = 0;
    size_t num_steps;
    ...
    mut_pop = (int *) malloc(sizeof(*mut_pop) * (size_t) num_demes);
    allele_frequency_mut = (double *) malloc(sizeof(*allele_frequency_mut) * (size_t) num_demes);
    final_mut_pop = (int *) malloc(sizeof(*final_mut_pop) * (size_t) num_demes);

    for (i = 0; i < num_demes; i++) {
        fread(&final_mut_pop[i], sizeof(int), 1, file);
    }

    for (i = 0; i < num_demes; i++) {
        fread(&mut_pop[i], sizeof(int), 1, file);
        allele_frequency_mut[i] = 1.0 * mut_pop[i] / tot_pop;
    }

    t_of_forward_ev = (double *) malloc(sizeof(*t_of_forward_ev) * num_steps);
    ev_type = (int *) malloc(sizeof(*ev_type) * num_steps);
    start_deme = (int *) malloc(sizeof(*start_deme) * num_steps);
    end_deme = (int *) malloc(sizeof(*end_deme) * num_steps);
    for (j = 1; j <= num_steps; j++) {
        fread(&t_of_forward_ev[num_steps - j], sizeof(double), 1, file);
        fread(&ev_type[num_steps - j], sizeof(int), 1, file);
        fread(&start_deme[num_steps - j], sizeof(int), 1, file);
    ...
    *tot_pop_ret = tot_pop;
    *migration_rate_ret = migration_rate;
    *final_mut_pop_ret = final_mut_pop;
    *mut_pop_ret = mut_pop;
    *allele_frequency_mut_ret = allele_frequency_mut;
    *t_of_forward_ev_ret = t_of_forward_ev;
    *ev_type_ret = ev_type;
    *start_deme_ret = start_deme;
    *end_deme_ret = end_deme;
    return ret;
}
...
msp_run_sweep(msp_t *self, FILE *file)
{
    ...
    double **ancestral_bg_pop_size = NULL;
    double tmp_rand = 0.0;
    double *p_coal_wild = NULL, *p_coal_mut = NULL, *p_mig_wild_right = NULL,
           *p_mig_mut_right = NULL, *p_mig_wild_left = NULL, *p_mig_mut_left = NULL,
           p_rec_wild = 0.0, p_rec_mut = 0.0;
    double p_any_ev = 0.0, p_cum_ev = 0.0;
    double t_start = 0.0, t_end = 0.0, t_of_next_ev = 0.0, t_current = 0.0;
    int num_demes = 0;
    double migration_rate = 0.0;
    double p_forward_ev = 0.0, t_of_next_forward_ev = 0.0;
    int tot_pop = 0;
    int *mut_pop = NULL, *final_mut_pop = NULL;
    ...
    ret = genic_selection_read_trajectory(file, &num_steps, &num_demes, &tot_pop, &migration_rate, &final_mut_pop, &mut_pop, &allele_frequency_mut, &t_of_forward_ev, &ev_type, &start_deme, &end_deme);
    ...
    ancestral_bg_pop_size = (double **) malloc(sizeof(double *) * (size_t) num_demes);
    for (i = 0; i < num_demes; i++) {
    ...
    while (msp_get_num_ancestors(self) > 1 && curr_step < num_steps
        ...
        for (j = 0; j < self->num_labels; j++) {
            ...
            for (i = 0; i < num_demes; i++) {
                ancestral_bg_pop_size[i][j]
                    = avl_count(&self->populations[i].ancestors[label]);
        ...
        for (i = 0; i < num_demes; i++) {
            if (ancestral_bg_pop_size[i][0] > 1 && allele_frequency_mut[i] != 1) {
                p_coal_wild[i] = ((ancestral_bg_pop_size[i][0]
                                      * (ancestral_bg_pop_size[i][0] - 1.0))
                                     * 0.5)
                                 / (tot_pop - mut_pop[i]);
            } else {
                p_coal_wild[i] = 0;
            }
            if (ancestral_bg_pop_size[i][1] > 1 && allele_frequency_mut[i] != 0) {
                p_coal_mut[i] = ((ancestral_bg_pop_size[i][1]
                                     * (ancestral_bg_pop_size[i][1] - 1.0))
                                    * 0.5)
                                / mut_pop[i];
            } else {
                p_coal_mut[i] = 0;
            ...
            if (num_demes > 1) {
                if (i > 0) {
                    p_mig_wild_left[i - 1] = migration_rate * ancestral_bg_pop_size[i][0]
                                             * (1.0 - allele_frequency_mut[i - 1]);
                    p_mig_mut_left[i - 1] = migration_rate * ancestral_bg_pop_size[i][1]
                                            * allele_frequency_mut[i - 1];
                }
                if (i < num_demes - 1) {
                    p_mig_wild_right[i] = migration_rate * ancestral_bg_pop_size[i][0]
                                          * (1.0 - allele_frequency_mut[i + 1]);
                    p_mig_mut_right[i] = migration_rate * ancestral_bg_pop_size[i][1]
                                         * allele_frequency_mut[i + 1];
                }
            }
        }
        ...
        if (t_current + t_of_next_ev >= t_of_next_forward_ev) {
            ...
            if (start_deme_index == end_deme[curr_step]) {
                ...
                if (curr_ev_type
                    == 0) { // mutant replaced with wildtype, mutant coalascence
                    p_forward_ev = (float) ancestral_bg_pop_size[start_deme_index][1]
                                   * (ancestral_bg_pop_size[start_deme_index][1] - 1.0)
                ...
                } else if (curr_ev_type
                           == 1) { // wildtype replaced with mutant, wt coalescence
                    p_forward_ev = (float) ancestral_bg_pop_size[start_deme_index][0]
                                   * (ancestral_bg_pop_size[start_deme_index][0] - 1.0)
                ...
                if (curr_ev_type == 2) { // wildtype replaced by mutant, mutant migration
                                         // + coalascence
                    p_forward_ev = 1.0 * ancestral_bg_pop_size[start_deme_index][1]
                                   / mut_pop[start_deme_index];
                    tmp_rand = gsl_rng_uniform(self->rng);
                    if (tmp_rand <= p_forward_ev) {
                        ...
                        p_forward_ev = 1.0
                                       * ancestral_bg_pop_size[end_deme[curr_step]][1]
                                       / mut_pop[end_deme[curr_step]];
                        tmp_rand = gsl_rng_uniform(self->rng);
                        if (tmp_rand <= p_forward_ev) {
                            ret = self->common_ancestor_event(
                                self, end_deme[curr_step], 1);
                        }
                    }
                    ...
                    p_forward_ev = 1.0 * ancestral_bg_pop_size[start_deme_index][0]
                                   / (tot_pop - mut_pop[start_deme_index]);
                    tmp_rand = gsl_rng_uniform(self->rng);
                    if (tmp_rand <= p_forward_ev) {
                        ret = msp_migration_event_in_background(
                        ...
                        p_forward_ev = 1.0
                                       * ancestral_bg_pop_size[end_deme[curr_step]][0]
                                       / (tot_pop - mut_pop[end_deme[curr_step]]);
                        tmp_rand = gsl_rng_uniform(self->rng);
                        if (tmp_rand <= p_forward_ev) {
                            ret = self->common_ancestor_event(
                                self, end_deme[curr_step], 0);
                        }
                    }
                    mut_pop[start_deme[curr_step]]++;
                    allele_frequency_mut[start_deme[curr_step]]
            ...
            if (num_demes > 1) {
                ...
                if (no_event_yet) {
                    for (i = 0; i < (num_demes - 1); i++) {
                        ...
                        if (tmp_rand < p_cum_ev / p_any_ev) {
                            ret = msp_migration_event_in_background(self, i + 1, i, 1);
                            no_event_yet = false;
                            break;
                        }
                    }
...
out:
    ...
    for (i = 0; i < num_demes; i++) {
        msp_safe_free(ancestral_bg_pop_size[i]);
    }
    msp_safe_free(ancestral_bg_pop_size);
    msp_safe_free(mut_pop);
    msp_safe_free(allele_frequency_mut);
    msp_safe_free(final_mut_pop);
    msp_safe_free(t_of_forward_ev);
    msp_safe_free(ev_type);
    msp_safe_free(start_deme);
    msp_safe_free(end_deme);
...
static double
msp_smc_k_get_common_ancestor_waiting_time(
    msp_t *self, population_id_t pop_id, label_id_t label)
{
    population_t *pop = &self->populations[pop_id];
    fenwick_t *mass_index;
     ...
     * become too large by rebuilding the indexing structure every
     * now and again. */
    mass_index = &self->populations[pop_id].coal_mass_index[label];
    tsk_bug_assert(mass_index != NULL);
    if (fenwick_rebuild_required(mass_index)) {
        fenwick_rebuild(mass_index);
        self->num_fenwick_rebuilds++;
    }
    lambda = fenwick_get_total(mass_index);

    return msp_get_common_ancestor_waiting_time_from_rate(self, pop, lambda);
}

static int
msp_smc_k_common_ancestor_event(
    msp_t *self, population_id_t population_id, label_id_t label)
{
    int ret = 0;
    ...
    /* RANDOM MASS PARANOIA */
    tsk_bug_assert(random_mass < num_pairs);
    tsk_bug_assert(random_mass >= 0);
    hull_id = fenwick_find(coal_mass_index, random_mass);
    ...
    x = (segment_t *) x_hull->lineage;
    x_node = avl_search(avl, x);
    tsk_bug_assert(x_node != NULL);
    avl_unlink_node(avl, x_node);
    y = (segment_t *) y_hull->lineage;
    y_node = avl_search(avl, y);
    tsk_bug_assert(y_node != NULL);
    y = (segment_t *) y_node->item;
    avl_unlink_node(avl, y_node);

    self->num_ca_events++;
    msp_free_hull(self, x_hull, population_id, label);
    msp_free_hull(self, y_hull, population_id, label);
    msp_free_avl_node(self, x_node);
    msp_free_avl_node(self, y_node);
    ret = msp_merge_two_ancestors(self, population_id, label, x, y, TSK_NULL, NULL);
...
int
msp_set_simulation_model_smc_k(msp_t *self, double hull_offset)
{
    int ret = 0;
