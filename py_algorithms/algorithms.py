...
class Simulator:
    ...
    def single_sweep_simulate(self):
        ...
        # print("allele_freqs",self._allele_freqs)
        ...
        while self.ancestors_remain() and sweep_traj_step < len(times) - 1:
            ...
            while event_prob > random.random() and sweep_traj_step < len(times) - 1:
                ...
                print(f"Sweep pop tot rate: {sweep_pop_tot_rate}")

                total_rate = sweep_pop_tot_rate
                if total_rate == 0:
                    break
                print(f"Event Probability: {1.0 - event_prob}")
            ...
            if self.t + e_time > self.modifier_events[0][0]:
                ...
                if random.random() < sweep_pop_tot_rate / total_rate:
                    ...
                    if r < e_sum / sweep_pop_tot_rate:
                        # coalescent in B
                        print("Event: Coalescent in B")
                        print(f"Probability of this event: {p_coal_B / sweep_pop_tot_rate}")
                        self.common_ancestor_event(0, 1)
                        ...
                        if r < e_sum / sweep_pop_tot_rate:
                            ...
                            if r < e_sum / sweep_pop_tot_rate:
                                # recomb in B
                                print("Event: Recombination in B")
                                print(f"Probability of this event: {p_rec_B / sweep_pop_tot_rate}")
                                self.hudson_recombination_event_sweep_phase(
                                    1, self.sweep_site, x
                                )
                            else:
                                # recomb in b
                                print("Event: Recombination in b")
                                print(f"Probability of this event: {(p_rec_b + p_rec_B - e_sum) / sweep_pop_tot_rate}")
                                self.hudson_recombination_event_sweep_phase(
                                    0, self.sweep_site, 1.0 - x
                                )
        ...
        print(f"Sweep Population Sizes Clean up: {sweep_pop_sizes}")   
