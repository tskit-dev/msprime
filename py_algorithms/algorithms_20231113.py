...
class Forward:
    def __init__(self, L, N, s, migration_matrix, tfinal, seed, l0=1):
    ...
    def next_gen(self, n):
        ...
        # Calculate the frequency of the allele in each deme
        x = np.array([float(n[i] / self.N) for i in range(self.L)])
        print('Pre-migration frequencies:', x)

        # Initialize an array to store the migrated allele frequencies
        xmig = np.zeros(self.L)
        current_size = np.zeros(self.L) + 1

        # Apply migration using the migration matrix
        for i in range(self.L):
            # Start with the pre-migration frequency
            xmig[i] = x[i]
            # current_size[i] = 1
            # Calculate the net effect of migration for deme i
            for j in range(self.L):
                if i != j:
                    # Influx and Outflow
                    influx = self.migration_matrix[i][j] * x[j]
                    influx_pop = self.migration_matrix[i][j]
                    outflow = self.migration_matrix[j][i] * x[i]
                    outflow_pop = self.migration_matrix[j][i]
                    xmig[i] += influx - outflow
                    current_size [i] += influx_pop - outflow_pop
                    print(f'Deme {i}: Influx from {j} = {influx}, Outflow to {j} = {outflow}')
                    if current_size[i] < 0:
                        raise Exception("DemeSize Can Not Be Negative")

        
        print('Current Size =', current_size)
                
        print('Xmig after migration:', xmig)

        # Apply selection and calculate the allele count for the next generation
        for i in range(self.L):
            p = (xmig[i] + self.s * xmig[i] * (1 - xmig[i]))/current_size[i]
            # Ensure p does not exceed 1
            p = min(p, 1)
            print('deme', i, 'p is', p)
            result[i] = np.random.binomial(self.N, p)
...
# Reorganizing the output of the Forward class to simulate the trajectory of allele frequencies in a backward fashion

class DeterministicTrajectorySimulator:
    def __init__(self, N, L, frequencies):
...
def main(args=None):
    ...
    #for _ in range(trials):
       # ts = run_simulate(args)
        ...
        #d = np.add(d, ts.diversity(windows=windows, mode='branch'))

   # final_windows = np.linspace(0, ts.sequence_length, num=100).tolist()
    #final_windows.remove(0)
   # d /= trials
   # plt.plot(final_windows, d)
   # plt.title('Diversity vs. Position - New Algorithm')
   # plt.xlabel('Position')
