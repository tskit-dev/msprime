...
class TestSLiMv5MutationModel:
    ...
    def test_constructor_errors(self):
        with pytest.raises(TypeError):
            _msprime.SLiMv5MutationModel()

        for bad_type in ["sdr", 0.222, None]:
            with pytest.raises(TypeError):
                _msprime.SLiMv5MutationModel(type=bad_type, next_id=234)
                _msprime.SLiMv5MutationModel(type=0, next_id=bad_type)
                _msprime.SLiMv5MutationModel(type=0, next_id=0, block_size=bad_type)

        with pytest.raises(_msprime.LibraryError):
            _msprime.SLiMv5MutationModel(type=-1, next_id=0)
            _msprime.SLiMv5MutationModel(type=1, next_id=-1)

    def test_uninitialised(self):
        model = _msprime.SLiMv5MutationModel.__new__(_msprime.SLiMv5MutationModel)
        with pytest.raises(SystemError):
            _ = model.type
        with pytest.raises(SystemError):
            _ = model.next_id
    ...
    def test_type(self):
        for mutation_type in [0, 10, 2**31 - 1]:
            model = _msprime.SLiMv5MutationModel(type=mutation_type)
            assert model.type == mutation_type
            assert model.next_id == 0

    def test_next_id(self):
        for next_id in [0, 10, 2**63 - 1]:
            model = _msprime.SLiMv5MutationModel(0, next_id=next_id)
            assert model.type == 0
            assert model.next_id == next_id

    def test_generation(self):
        model = _msprime.SLiMv5MutationModel(0)
...
class TestSimMutations:
    ...
    def test_slim_v5_model(self):
        rng = _msprime.RandomGenerator(1)
        imap = uniform_rate_map(1)
        tables = _msprime.LightweightTableCollection(1.0)
        model = _msprime.SLiMv5MutationModel(1234, 5678)
