...
class BranchStatsTestCase(unittest.TestCase):
    ...
    def test_case_1(self):
        ...
        # branch lengths:
        self.assertAlmostEqual(msprime.branch_length_diversity(ts,[0],[1]),true_diversity_01)
        self.assertAlmostEqual(msprime.branch_stats(ts,A,f),true_diversity_01)
        self.assertAlmostEqual(msprime.branch_stats_node_iter(ts,A,f),true_diversity_01)

        # mean diversity between [0,1] and [0,2]:
        true_mean_diversity = (0 + true_diversity_02 + true_diversity_01 + true_diversity_12)/4
        A = [ [0,1], [0,2] ]
        n = [ len(a) for a in A ]
        def f(x):
            return float(x[0]*(n[1]-x[1]) + (n[0]-x[0])*x[1])/4.0
        # branch lengths:
        self.assertAlmostEqual(msprime.branch_length_diversity(ts,A[0],A[1]),true_mean_diversity)
        self.assertAlmostEqual(msprime.branch_stats(ts,A,f),true_mean_diversity)
        self.assertAlmostEqual(msprime.branch_stats_node_iter(ts,A,f),true_mean_diversity)
