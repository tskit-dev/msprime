...
- Changes to ``SLiMMutationModel`` to match changes in SLiM 6.0: this mutation
  model no longer has the ``type`` and ``slim_generation`` arguments, and stores
  different information in metadata (the list of mutation IDs, in binary). Mutations
  generated by msprime >= 1.4.3 will require adding metadata (e.g., with
  pyslim) before they can be read in by SLiM. ({issue}`2534`, {pr}`2535`,
  {user}`petrelharp`)
